#!/usr/bin/env bash
# LocateAnything full SFT (SDPA, no Magi).
#
# Usage:
#   conda activate locateanything
#   bash shell/locate-anything-streaming-sdpa.sh [NNODES] [OUTPUT_DIR]
# Foreground:
#   TRAIN_BACKGROUND=0 bash shell/locate-anything-streaming-sdpa.sh
set -euo pipefail
...
# Paths / model / data
# =============================================================================
MODEL_PATH=${MODEL_PATH:-"/workspace/models/CommonModels/LocateAnything-3B"}
META_PATH=${META_PATH:-"data/pcb_dimension/recipe_synth_v4.json"}
...
FREEZE_BACKBONE=${FREEZE_BACKBONE:-False} # vision_model (ViT)

# =============================================================================
# Learning rate
# actual_lr = LR * scale
# default @ LR=2e-5 → ViT 5e-6 | MLP 2e-5 | LLM 1e-5
# =============================================================================
LR=${LR:-1e-5}
...
# Prefer explicit WARMUP_STEPS; else derive from MAX_STEPS * WARMUP_RATIO
if [ -z "${WARMUP_STEPS:-}" ]; then
  WARMUP_STEPS="$(python -c "print(max(1, int(${MAX_STEPS} * ${WARMUP_RATIO})))")"
fi
SAVE_STEPS=${SAVE_STEPS:-100}
SAVE_LIMIT=${SAVE_LIMIT:-3}
# Copy to output_dir/milestones every N steps (0=disable). Prefer multiple of SAVE_STEPS.
MILESTONE_INTERVAL=${MILESTONE_INTERVAL:-2000}

# =============================================================================
# Batch / packing / seq length (SDPA-safe defaults)
# =============================================================================
...
# =============================================================================
# wandb  (same style as pad_detect qwen3-vl-4b-full-syn4.0-tsv.sh)
# =============================================================================
...
export WANDB_PROJECT="${WANDB_PROJECT:-size_line_value_match}"
export WANDB_NAME="${WANDB_NAME:-locateanything-3b-full-pcb-sdpa}"
export WANDB_RESUME="${WANDB_RESUME:-allow}"

# =============================================================================
# Runtime env
# =============================================================================
# HF_TOKEN only needed if MODEL_PATH is a Hub id
if [ -n "${HF_TOKEN:-}" ]; then
  export HF_TOKEN
fi
...
if [ "${TRAIN_BACKGROUND:-1}" != 0 ] && [ "${_SDPA_TRAIN_INNER:-0}" != 1 ]; then
  export _SDPA_TRAIN_INNER=1
  # Use absolute SCRIPT_PATH: we already cd'd into ROOT, so relative $0 breaks.
...
if [ "${_SDPA_TRAIN_INNER:-0}" = 1 ] || [ "${TRAIN_LAUNCHER_MANAGED:-0}" = 1 ]; then
  run_train
else
