#!/usr/bin/env bash
# LocateAnything Magi SFT continue-train: MoonViT-V2 synth ckpt → real anno mix.
#
# Typical flow:
#   1) bash shell/locate-anything-streaming-synthdata-moonvitv2.sh
#   2) after synth finishes, run this script (defaults assume checkpoint-6000)
#
# Usage:
#   conda activate locateanything
#   bash shell/locate-anything-streaming-synthdata-annodata-moonvitv2.sh [NNODES] [OUTPUT_DIR]
# Foreground:
#   TRAIN_BACKGROUND=0 bash shell/locate-anything-streaming-synthdata-annodata-moonvitv2.sh
set -euo pipefail
...
# Keep distinct from v1 annodata (29500) and v2 synth (29501).
PORT=${PORT:-29502}
...
echo "============================================================"
echo " LocateAnything Magi SFT continue (MoonViT-V2 synth → anno)"
echo "------------------------------------------------------------"
